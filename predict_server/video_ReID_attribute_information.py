...
class VideoBaseline(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice,
                 fusion_method="baseline", attr_lens=[], attr_loss="bce"):
        ...
        if len(self.attr_lenes) != 0:
            if self.attr_loss == "mce":
                ...
                self.attr_on = True
                self.attention_fusion1 = nn.Parameter(
                    torch.Tensor(np.ones([1, len(self.attr_lenes[0]), 16, 8], dtype=float)))
                self.attention_fusion2 = nn.Parameter(
                    torch.Tensor(np.ones([1, len(self.attr_lenes[1]), 16, 8], dtype=float)))
    ...
    def forward(self, x):
        ...
        if self.fusion_method == "ATTR_TA":
            if self.attr_loss == "mce":
                ...
                global_feat1 = attr_a1 * global_feat.view(b, t, self.in_planes, global_feat.size(2),
                                                          global_feat.size(3))
                global_feat1 = global_feat1.view(b * t, self.in_planes, global_feat.size(2), global_feat.size(3))

                global_feat2 = attr_a2 * global_feat.view(b, t, self.in_planes, global_feat.size(2),
                                                          global_feat.size(3))
...
def load_model():
    model_path = '/Users/huangtehui/PycharmProjects/flask_apps/resnet50-19c8e357.pth'
    model_name = 'resnet50'
    last_stride = 1
    num_classes = 625
    neck = 'bnneck'
    ...
    fusion_method = 'ATTR_TA'
    attr_len = [[5, 6, 2, 2, 2, 2, 2, 2, 2], [9, 10, 4, 2, 2, 2, 2, 2, 2, 2, 2, 2]]
    model = VideoBaseline(num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice,
                          fusion_method=fusion_method, attr_lens=attr_len)
    pre_model_path = '/Users/huangtehui/PycharmProjects/Video-based-person-ReID-with-Attribute-information/logs/2024-01-08_14-54-45/rank1_1.5035538531057626_checkpoint_ep80.pth'
